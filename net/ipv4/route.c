...
restart:
	...
	if (!rt_caching(dev_net(rt->u.dst.dev))) {
		...
		if (rt->rt_type == RTN_UNICAST || rt->fl.iif == 0) {
			/*绑定路由缓存到下一跳*/
			int err = arp_bind_neighbour(&rt->u.dst);//负责为路由缓存项创建邻居项（struct neighbour）并与之绑定。 
			if (err) {
				if (net_ratelimit())
					printk(KERN_WARNING
	...
	/*找到struct rtable 对应的hash桶，并存放进去 */
			     ...
			     unsigned flags)
{
	...
	if (err == 0) {
		...
		err = rt_intern_hash(hash, rth, rp, NULL);//主要负责将构建好的路由缓存项加入路由缓存的hash表中
	}
...
make_route:
	err = ip_mkroute_output(rp, &res, &fl, oldflp, dev_out, flags);//根据路由表的查找结果构建一个路由缓存项
			  ...
			  const struct flowi *flp)
{
	...
	rcu_read_lock_bh();
	for (rth = rcu_dereference(rt_hash_table[hash].chain); rth;
		rth = rcu_dereference(rth->u.dst.rt_next)) {//根据flowi结构查找路由缓存，如果找着到则通过rp返回
			 ...
			 struct sock *sk, int flags)
{
	...
	if ((err = __ip_route_output_key(net, rp, flp)) != 0)//进行路由缓存的查找
		return err;
