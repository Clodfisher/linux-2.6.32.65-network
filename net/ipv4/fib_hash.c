...
//fib_node实例代表每一个唯一的目的网络的路由表项，即同一个子网中所有路由表项所共享的信息。目的网络相同
//但其他配置参数不同的路由表项共享同一个fib_node实例，因此一个fib_node实例上存在着一个或多个路由表项。
struct fib_node {
	struct hlist_node	fn_hash;
	struct list_head	fn_alias;  //fn_alias指向一个或多个fib_alias结构实例构成的链表
	__be32			fn_key;        //由IP地址和路由项的netmask与操作后得到，被用作查找路由表时的搜索条件
	struct fib_alias        fn_embedded_alias;//嵌入
};

//一个zone是一组有着相同目的地址掩码长度的路由表项的散列表 - yht -
struct fn_zone {
	struct fn_zone		*fz_next;	/* Next not empty zone	*///将活动的zone连接在一起的指针，该链表的头部存储在fn_hash(tb->tb_data)数据结构的fn_zone_list字段中
	struct hlist_head	*fz_hash;	/* Hash table pointer	*///指向存储该zone中路由项的散列表,struct hlist_head head = &fz->fz_hash[fn_hash(k, fz)];//fib_node为链接元素的hash表
	int			fz_nent;	/* Number of entries	*///在该zone的散列表中fib_node实例的数目，用于检查是否需要改变散列表的容量。

	int			fz_divisor;	/* Hash divisor		*///表示散列表fz_hash的容量，以及散列表桶的数目。
	u32			fz_hashmask;	/* (fz_divisor - 1)	*///其值为fz_divisor - 1，用来计算散列表的关键值
#define FZ_HASHMASK(fz)		((fz)->fz_hashmask)

	int			fz_order;	/* Zone order		*///网络掩码的长度，255.255.255.0的网络掩码长度为24
	__be32			fz_mask;                      //网络掩码
